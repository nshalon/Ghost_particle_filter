...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        expectedGhostPositions = []
        expectedGhostDistances = []
        for belief in livingGhostPositionDistributions:
            maxProb = 0
            for pos, prob in belief.items():
                if prob > maxProb:
                    maxProb = prob
                    expectedGhostPos = pos
            expectedGhostPositions.append(expectedGhostPos)
            # expectedGhostDistances[i]=self.distancer.getDistance(expectedGhostPos, pacmanPosition)
        minDist = float("inf")
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            for ghostPos in expectedGhostPositions:
