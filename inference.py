...
class ExactInference(InferenceModule):
  ...
  """
    ...
    """
        ...
        for state, priorprob in self.beliefs.items():
            # print(state,priorprob)
            dist = util.manhattanDistance(state, pacmanPosition)
            self.beliefs[state] = emissionModel[dist] * priorprob
        self.beliefs.normalize()
    ...
    """
        ...
        for oldPos, priorprob in self.beliefs.items():
            # print(oldPos,priorprob)
            if oldPos in self.legalPositions:
                # print(self.setGhostPosition(gameState, oldPos))
                newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
                for newPos, prob in newPosDist.items():
                    tempBeliefs[newPos] = tempBeliefs[newPos] + prob * priorprob
...
class ParticleFilter(InferenceModule):
  ...
  """
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = [random.choice(self.legalPositions) for particle in range(self.numParticles)]
    ...
    """
        ...
        particleProbs = util.Counter()
        for particle in self.particles:
            dist = util.manhattanDistance(particle, pacmanPosition)
            particleProbs[particle] += emissionModel[dist]

        particleProbs.normalize()

        if noisyDistance == None: # jail case
            self.particles = [self.getJailPosition() for i in range(self.numParticles)]
        elif particleProbs.totalCount() != 0: # resample based on weights when sum != 0
            self.particles = [util.sample(particleProbs) for i in range(self.numParticles)]
        else: # resample from uniform when sum of weights = 0
            self.initializeUniformly(gameState)
    ...
    """
        ...
        newPosDists = [self.getPositionDistribution(self.setGhostPosition(gameState, oldPos)) for oldPos in self.particles]
        self.particles = [util.sampleFromCounter(dist) for dist in newPosDists]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for part in range(self.numParticles):
            self.particles.append(tuple([random.choice(self.legalPositions) for ghost in range(self.numGhosts)]))
    ...
    """
        ...
        particleProbs = util.Counter()
        for particle in self.particles:
            distances = [util.manhattanDistance(ghostPosition,pacmanPosition) for ghostPosition in particle]
            factor = 1
            for ghostNum in range(self.numGhosts):
                emissionModel = emissionModels[ghostNum]
                distance = distances[ghostNum]
                factor = factor * emissionModel[distance]
            particleProbs[particle] += factor

        particleProbs.normalize()

        jail = [False] * self.numGhosts
        for ghostNum in range(self.numGhosts): # find which ghosts are in jail
            if noisyDistances[ghostNum] == None:
                jail[ghostNum] = True

        sumAllParticles = particleProbs.totalCount()
        if sumAllParticles != 0:
            self.particles = [util.sampleFromCounter(particleProbs) for i in range(self.numParticles)]
        else:
            self.initializeParticles()
        ...
        for particle in self.particles:
            ...
            for ghostNum in range(self.numGhosts):
                if jail[ghostNum]:
                    particle[ghostNum] = self.getJailPosition(ghostNum)
            particlesAfterJail.append(tuple(particle))

        self.particles = particlesAfterJail
